...
// same minute.
//
// This file exports ONLY its default component -- trainKey lives in
// utils/trainKey.js, since a file mixing a component export with a
// plain-function export can't be cleanly Fast-Refreshed by Vite (see
// that file's own comment for the bug this caused).
...
// Determines which single hour cell should show a block's department-
// initial label, computed as a pure, read-only lookup from `blocks`
// itself rather than a mutable Set built up as a side effect during
// render. A stateful "have I already labeled this task_id" Set mutated
// inside HourCell (shownTaskIds.add(...)) breaks under React 18
// StrictMode (enabled in main.jsx): StrictMode deliberately double-
// invokes render functions to catch exactly this kind of impurity, so
// the second invocation of an hour's HourCell would see the Set already
// containing its own task_id from the first, throwaway invocation --
// even the block's own first hour would render as "already labeled" and
// show nothing. Computing the lookup fresh each time (same input, same
// answer, no shared mutable state) is safe to run twice under
// StrictMode.
...
function HourCell({ date, blocks, trains, hour, onBlockClick, onTrainClick, labelHourByTask, highlightedTrains }) {
  ...
  // Colored by department like any other block, not a special "combined"
  // color. Shows the department initial (same convention the normal
  // per-segment cells below use); the tooltip/details modal already say
  // "negotiated exception" clearly enough without a dedicated icon.
    ...
    <td>
      <div style={{ display: "flex", width: "100%", height: "100%" }}>
        {pieces.map((p, i) => {
          ...
          const b = p.data;
          // An emergency block gets its own distinct alert color instead
          // of a department color -- it's not routine departmental work,
          // and should read as an "EMERGENCY SITUATION" at a glance.
