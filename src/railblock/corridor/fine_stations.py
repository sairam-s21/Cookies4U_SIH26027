"""Fine-grained corridor station model.

Block sections derived from major-station stop patterns are coarse: one
saturated long section (e.g. MAS-AJJ, 68 km) blocks scheduling across its
whole length even when only part of it is actually busy. Train 12243's
full stop-by-stop timetable (see fine_stations_source.py) gives real
points on this exact corridor -- including signal cabins and block huts,
which are genuine block-boundary points, not noise -- letting block
sections be rebuilt at a granularity close to the real physical
signalling layout instead of just major stations.

This module's scope matches fine_stations_source.py's point list
(currently MAS-JTJ).
...
independently-derived coarse station list (train-union method over
...
def load_fine_corridor_stations() -> pd.DataFrame:
    ...
    JTJ inclusive, ordered by distance."""
    rows = [
        {
            "sequence_order": i + 1,
            "station_code": code,
