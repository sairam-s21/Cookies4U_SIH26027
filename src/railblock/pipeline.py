...
occupancy, automatic task splitting). Session 30: the former Option 3
(bounded negotiated exceptions) was removed along with train
cancellation/negotiation, now out of scope for this project.
...
caller (an API endpoint in a later session, or a demo script) would.
"""
...
def load_corridor(corridor_stations_path=None, granularity: str = "fine") -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    granularity="fine" (default, Session 4): the 102-point/101-section
    model derived from train 12243's real timetable
    (railblock.corridor.fine_stations) -- materially more schedulable
    capacity than the coarse model, see PROGRESS.md Session 4's
    fragmentation before/after. This is what the scheduler uses going
    forward.
    granularity="coarse": the original Session 1 19-station/18-section
    model (train-union derivation over Train_details_22122017.csv),
    kept available for comparison/regression testing, not for new demos.
...
def run_end_to_end(
    ...
    """Run the full Session 1 (data) -> Layer 2 -> Layer 3 (Options 1-2)
    pipeline on a fresh (or seeded) synthetic scenario for the real MAS-JTJ
    corridor (Session 16 scope reduction). `passenger_occupancy` already reflects Option 1's realistic
    ...
    `granularity`: see load_corridor -- "fine" (default) is the Session 4
    102-point model now used going forward; "coarse" reproduces Session
    1-3's original 19-station model for comparison.

    `demand_scenario`: Session 5 -- if given ("stress_test" or
